...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-accent flex justify-around items-center gap-10 p-10 lg:flex-col lg:pb-48 relative z-[1]">
      <div className="flex gap-10">
        {FooterLinks.map((link, index) => {
          const popupContent = (
            <div
              className={`p-32 lg:p-16  rounded-3xl dark-shadow relative max-h-[80vh] overflow-y-auto ${
                theme === 'dark' ? 'bg-success' : 'bg-white'
              }`}
            >
              <button
                className="fixed top-0 right-0 bg-blue p-4 z-10 rounded-2xl m-4 hover:bg-base-200 transition-all duration-500 dark-shadow text-white"
                onClick={() => {
                  close()
                }}
              >
